...
public class SolicitudPrestamoController {
    ...
    @Transactional
    @PostMapping
    public ResponseEntity<SolicitudPrestamoDTO> crearPrestamo(@RequestBody CrearSolicitudPrestamoDTO request) {
        ...
        dto.add(linkTo(methodOn(SolicitudPrestamoController.class).buscarPorId(guardada.getIdPrestamo())).withSelfRel());
    ...
    public ResponseEntity<CollectionModel<SolicitudPrestamoDTO>> listar() {
        ...
        return ResponseEntity.ok(CollectionModel.of(dtos, linkTo(methodOn(SolicitudPrestamoController.class).listar()).withSelfRel()));
    }

    @GetMapping("/{id}")
    public ResponseEntity<SolicitudPrestamoDTO> buscarPorId(@PathVariable Long id) {
        return solicitudService.buscarPorId(id)
                .map(solicitud -> {
                    SolicitudPrestamoDTO dto = mapToDTO(solicitud);
                    dto.add(linkTo(methodOn(SolicitudPrestamoController.class).listar()).withRel("prestamos"));
                    return ResponseEntity.ok(dto);
                })
    ...
    public ResponseEntity<Void> devolverPrestamo(@PathVariable Long id) {
        // 1. Registra fecha de devolución en los detalles (Items)
        detalleService.registrarDevolucion(id);
        
        // 2. IMPORTANTE: Cambia el estado de la solicitud a "Devuelto"
        solicitudService.finalizarPrestamo(id);

        return ResponseEntity.ok().build();
    }

    @GetMapping("/{id}/detalles")
    public ResponseEntity<List<DetallePrestamoDTO>> verDetalles(@PathVariable Long id) {
        List<DetallePrestamoDTO> detalles = detalleService.buscarPorSolicitud(id).stream()
